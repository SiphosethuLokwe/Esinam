...
if(!file_exists($_FILES['FileUpload']['tmp_name']))
	{	
		$path = 'assets/images/default-blog-thumb.PNG';
        ...
        echo '<script type="text/javascript">
                    window.location = "newpost.html"?msg='.$msg.'
               </script>';
		...
		if (in_array($ext,$allowed)){
			...
			$image = file_get_contents($_FILES[$path]['tmp_name']);
			$result=$article_datamaper->SaveArticle($article_title,$description,$image,$date,$Con,$Comm);
			...
			if($result)
        {
            ...
            $msg = $msg.'msg="File must be a jpg,png,jpeg';
                echo '<script type="text/javascript">
                    window.location = "newpost.html.php"?msg='.$msg.'
               </script>';
			header('Location:'.$loginurl.'?'.$msg);
		}
